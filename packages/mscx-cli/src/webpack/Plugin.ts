...
class Plugin {
  async setup() {
    const {wpChain, isDev, root, src, options, cacheFiles} = mscxEnv
    ...
    // TS/ESLint
    const tsconfig = mscxEnv.resolve('tsconfig.json')
  ...
  private get html() {
    const {template, favicon, isDev} = mscxEnv
