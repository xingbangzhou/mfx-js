...
module.exports = (env, config, args, afegConfigPath) => {
  const { entry, appSrc, dist } = getPaths()
  const isDev = env === 'development'
  const buildDependenciesConfigs = [__filename]
  if (afegConfigPath) {
    buildDependenciesConfigs.push(afegConfigPath)
  }
  const commonConfig = {
    cache: {
      version: `${version}-${gitVersion}${args.hot ? '-hot' : ''}${args.afegEnv ? '-' + args.afegEnv : ''}`,
      type: 'filesystem',
