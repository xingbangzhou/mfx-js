...
const checkRemote = async () => {
  const remoteConfig = resolveApp('afeg-config.js')
  const [isRemoteConfig, isRemotePackageJson] = await Promise.all([
    fs.exists(remoteConfig),
    fs.exists(appPackageJson)
  ])
  const afegPackageJsonPath = isRemotePackageJson ? appPackageJson : null
  let afegConfigPath = isRemoteConfig ? remoteConfig : null
  return {
    afegPackageJsonPath,
    afegConfigPath
  }
